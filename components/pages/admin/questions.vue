  ...
  <div class="d-flex flex-column">
    ...
    <div class="d-flex flex-column gap-5">
      <text-field
        ...
        :disabled="updating"
        :loading="retrieving"
        placeholder="Give your questionnaire a name"
        >Title</text-field
      >

      <v-switch
        v-model="internalSalary"
        class="wfc"
      ...
      <template v-if="!retrieving">
        <div
          v-for="(q, i) in internalQuestions"
          :key="q.id"
      ...
      <v-btn
        ...
        plain
        width="220"
        :disabled="retrieving || updating"
        @click="addQuestion"
      >
        <v-icon left class="primary--text">mdi-plus-circle-outline</v-icon>
        <div
          class="link-btn"
          :class="retrieving || updating ? 'disabled--text' : 'primary--text'"
        >
...
export default Vue.extend({
  ...
  computed: {
    retrieving(): boolean {
      return (
        useQuestionnaireStore().loading ||
        useQuestionnaireStore().questionsLoading
      )
    },
    updating(): boolean {
      return (
        useQuestionnaireStore().updating ||
        useQuestionnaireStore().questionsUpdating
      )
    },
